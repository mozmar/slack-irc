...
const ALLOWED_SUBTYPES = ['me_message', 'bot_message', 'file_share'];
const ALERT_SUBTYPES = ['bot_message', 'file_share'];
...
class Bot {
  constructor(options) {
    ...
    this.disable_irc_to_slack = options.disable_irc_to_slack || false;
    this.disable_slack_to_irc = options.disable_slack_to_irc || false;
  ...
  attachListeners() {
    ...
    this.slack.rtm.on('message', (message) => {
      ...
      if (message.type === 'message' &&
        ...
        logger.debug('ignored message', message.type, message.subtype, message.text)
      }
    });
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        const prelude = `Command sent from Slack by ${username}:`;
        this.ircClient.say(ircChannel, prelude);
      } else if (message.subtype === 'me_message') {
        text = `Action: ${username} ${text}`;
      } else if (text) {
        text = `<${username}> ${text}`;
        if (message.subtype && ALERT_SUBTYPES.includes(message.subtype)) {
          text = `BOT_ALERT: ${text}`;
        }
      }
      ...
      if (message.attachments) {
        message.attachments.forEach(attachment => {
          this.ircClient.say(ircChannel, `BOT_ALERT: <${username}> ${attachment.fallback}`)
        })
      }
    }
  }
